...
class MethodsPreprocessor {
    ...
    func getAllModule() {
        var commonPrefix = self.filePathMap.first!.value;
        for (file, path) in self.filePathMap {
            commonPrefix = commonPrefix.commonPrefix(with: path)
        }
        for (file, path) in self.filePathMap {
            
            let leftPath = path[commonPrefix.count...]
            let components = leftPath.split(separator: Character("/"))
            var moduleName:String = String(components[0])
